...
import { Plus, Search, Edit2, Trash2, Filter, ArrowUpRight, AlertTriangle } from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Gerencie suas entradas financeiras de forma simples</p>
        </div>
        <Button onClick={handleAdd} className="bg-blue-600 hover:bg-blue-700 active:scale-95 rounded-2xl h-12 px-6 gap-2 shadow-lg shadow-blue-100 transition-all hover:shadow-xl hover:-translate-y-0.5 w-full sm:w-auto justify-center">
          <Plus size={18} />
          Nova Receita
        </Button>
      </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center">
            <div className="relative flex-1 w-full">
              ...
              <Input 
                ...
                className="pl-10 rounded-2xl h-11 border-slate-200 focus-visible:ring-2 focus-visible:ring-blue-500/20 focus-visible:border-blue-500 bg-slate-50/50 w-full"
                value={searchTerm}
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
              ...
              <div className="bg-emerald-50/60 px-5 py-2 rounded-2xl flex flex-col justify-center border border-emerald-100/50 w-full sm:w-auto text-center sm:text-left shrink-0">
                <span className="text-[9px] font-bold text-emerald-600 uppercase tracking-wider">Total Filtrado</span>
                <span className="text-base font-bold text-emerald-700 whitespace-nowrap">{formatCurrency(totalIncomes)}</span>
              </div>
            </div>
          </div>
                  ...
                  {incomes.map((t) => {
                    ...
                    return (
                      <tr key={t.id} className="group hover:bg-slate-50/40 transition-colors">
                        ...
                        <td className="py-4">
                          <Badge variant="secondary" className="bg-blue-50/50 text-blue-600 border border-blue-100/30 hover:bg-blue-50 rounded-lg font-medium text-xs px-2.5 py-0.5 whitespace-nowrap">
                            {category?.name || 'Sem categoria'}
                          </Badge>
                        </td>
                        <td className="py-4 text-slate-400 text-xs font-medium whitespace-nowrap">{formatDate(t.date)}</td>
                        <td className="py-4 font-bold text-emerald-600 text-sm whitespace-nowrap">{formatCurrency(t.amount)}</td>
        ...
        <AlertDialogContent className="w-[95%] sm:max-w-md rounded-3xl border border-slate-100 shadow-2xl">
          <AlertDialogHeader className="flex flex-col items-center text-center space-y-3">
            ...
            <div className="space-y-1">
              <AlertDialogTitle className="text-lg font-bold text-slate-900">Deseja mesmo apagar esta receita?</AlertDialogTitle>
              <AlertDialogDescription className="text-slate-500 text-sm">
                Esta ação é permanente e removerá o registro de receita do seu histórico financeiro. Não será possível recuperá-lo.
              </AlertDialogDescription>
            </div>
          </AlertDialogHeader>
